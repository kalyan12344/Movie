...
const SeatsBooking = () => {
    ...
    const Seat = ({ seatData, onSelectSeat }) => {
        ...
        const seatStatusClass = is_selected
            ? 'selected'
            : 'reserved'; // Assuming is_selected=true means reserved, modify this based on your backend logic
        ...
        return (
            <div className="seat-layout">
                {availableSeats.map((seat, index) => (
                    <Seat key={index} seatData={seat} onSelectSeat={onSeatSelect} />
                ))}
            </div>
        );
    };
    const handleSeatSelect = (selectedSeat) => {
        ...
        const newSelectedSeats = updatedSeatDatanew.filter((seat) => seat.is_selected);
    ...
    const handleProceedToBooking = () => {
        if (selectedSeats.length > 0) {
            ...
            Axios.post(`http://localhost:8080/ticket/get`,{showtime_id,selectedSeats}).then((response)=>{
              console.log(response)  
            var total_price=response.data[0].total_price
            if (total_price==null){
total_price=0
            }
            const confirmationMessage = `For booking of the seats: ${selectedSeats.map((seat) => seat.seat_name).join(', ')} The toal price is ${total_price} `;
            if (window.confirm(confirmationMessage)) {
                ...
                navigate(`/reservation/${showtime_id}/${total_price}/${encodedObject}`)
    ...
    return (
        <div>
                ...
                {selectedSeats.length > 0 ? (
                    <ul>
                        {selectedSeats.map((seat) => (
                            <li key={seat.seat_id}>{seat.seat_name}</li>
                        ))}
                    </ul>
                ) : (
            ...
            <button onClick={handleProceedToBooking} disabled={selectedSeats.length === 0}>
                Proceed to Booking
            </button>
        </div>
    );
